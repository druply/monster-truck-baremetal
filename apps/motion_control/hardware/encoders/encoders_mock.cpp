...
void Encoders::monitor_encoder_right_thread(void) noexcept {
    ...
    while(m_running.load(std::memory_order_relaxed)) {
        ...
        m_right_encoder.fetch_add(1, std::memory_order_release);
        int64_t ts = duration_cast<nanoseconds>(steady_clock::now().time_since_epoch()).count();
        m_right_timestamp_ns.store(ts, std::memory_order_relaxed);
    }
    ...
    int64_t Encoders::get_left_pulses() const noexcept  { return m_left_encoder.load(std::memory_order_acquire); }
    int64_t Encoders::get_right_pulses() const noexcept  { return m_right_encoder.load(std::memory_order_acquire); }
...
void Encoders::monitor_encoder_left_thread(void) noexcept {
    ...
    while(m_running.load(std::memory_order_relaxed)) {
        ...
        m_left_encoder.fetch_add(1, std::memory_order_release);
        int64_t ts = duration_cast<nanoseconds>(steady_clock::now().time_since_epoch()).count();
        m_left_timestamp_ns.store(ts, std::memory_order_relaxed);
    }
